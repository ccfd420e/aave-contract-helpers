...
export class UiPoolDataProvider {
     ...
     */
    async getReservesList({ lendingPoolAddressProvider }, overrides) {
        if (!isAddress(lendingPoolAddressProvider)) {
            throw new Error('Lending pool address is not valid');
        }
        return this._contract.getReservesList(lendingPoolAddressProvider, overrides);
    }
    /**
     * Get data for each lending pool reserve
     */
    async getReservesData({ lendingPoolAddressProvider }, overrides) {
        if (!isAddress(lendingPoolAddressProvider)) {
            throw new Error('Lending pool address is not valid');
        }
        return this._contract.getReservesData(lendingPoolAddressProvider, overrides);
    }
    /**
     * Get data for each user reserve on the lending pool
     */
    async getUserReservesData({ lendingPoolAddressProvider, user }, overrides) {
        if (!isAddress(lendingPoolAddressProvider)) {
            throw new Error('Lending pool address is not valid');
        }
        if (!isAddress(user)) {
        ...
        return this._contract.getUserReservesData(lendingPoolAddressProvider, user, overrides);
    }
    async getReservesHumanized({ lendingPoolAddressProvider }, overrides) {
        const { 0: reservesRaw, 1: poolBaseCurrencyRaw } = await this.getReservesData({ lendingPoolAddressProvider }, overrides);
    ...
    async getUserReservesHumanized({ lendingPoolAddressProvider, user }, overrides) {
        const { 0: userReservesRaw, 1: userEmodeCategoryId } = await this.getUserReservesData({ lendingPoolAddressProvider, user }, overrides);
        return {
    ...
    async getEModes({ lendingPoolAddressProvider }, overrides) {
        if (!isAddress(lendingPoolAddressProvider)) {
            throw new Error('Lending pool address is not valid');
        }
        return this._contract.getEModes(lendingPoolAddressProvider, overrides);
    }
    async getEModesHumanized({ lendingPoolAddressProvider }, overrides) {
        if (!isAddress(lendingPoolAddressProvider)) {
            throw new Error('Lending pool address is not valid');
        }
        const eModeData = await this.getEModes({ lendingPoolAddressProvider }, overrides);
